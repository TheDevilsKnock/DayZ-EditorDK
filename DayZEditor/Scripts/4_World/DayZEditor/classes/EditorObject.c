...
{
	// --- REFACTOR NOTICE ---
	// This class is being actively refactored into an ECS architecture.
	// Its responsibilities are being moved to Components and Systems.
	// It currently serves as a bridge between the old and new systems.

	// --- NEW ECS BRIDGE ---
	private EcsEntity m_EcsEntityId;
	...
	protected bool m_IsSelected; // DEPRECATED: To be replaced by SelectionComponent
	...
	// MODIFIED CONSTRUCTOR: Now accepts the EcsEntity ID to establish the link.
	void EditorObject(notnull EditorObjectData data, EcsEntity entityId)
	{
		EditorLog.Trace("EditorObject " + data);
		m_Data = data;
		m_EcsEntityId = entityId; // Store the link to the ECS entity.

		// The object creation is handled by EditorObjectManagerModule before this constructor is called.
		// We now just assign the pre-created object from the data payload.
	...
	void ~EditorObject()
	{
		...
		// The corresponding EcsEntity will be destroyed by the EditorObjectManagerModule
		// after this EditorObject instance is destroyed.
	...
	/**
	 * @brief Gets the ECS entity ID associated with this object.
	 * @return The EcsEntity ID.
	 */
	EcsEntity GetEcsEntity()
	{
		return m_EcsEntityId;
	}
