class EditorBuildMenuTaxonomy: Managed
{
	protected static ref array<ref DayZEditorBuildMenuTabRegistration> s_BuiltinTabs;
	protected static ref array<ref DayZEditorBuildMenuSubcategoryRegistration> s_BuiltinSubcategories;
	protected static ref map<string, bool> s_BuiltinSubcategoryKeys;

	protected ref array<ref EditorBuildMenuTabDefinition> m_Tabs = {};
	protected ref array<ref EditorBuildMenuSubcategoryDefinition> m_Subcategories = {};
	protected ref map<string, ref EditorBuildMenuTabDefinition> m_TabById = new map<string, ref EditorBuildMenuTabDefinition>();
	protected ref map<string, ref EditorBuildMenuSubcategoryDefinition> m_SubcategoryByKey = new map<string, ref EditorBuildMenuSubcategoryDefinition>();
	protected ref map<string, ref EditorBuildMenuSectionDefinition> m_SectionByKey = new map<string, ref EditorBuildMenuSectionDefinition>();
	...
	{
		EnsureBuiltinDefinitions();
		foreach (DayZEditorBuildMenuTabRegistration tab_registration: s_BuiltinTabs) {
	...
	protected void SeedBuiltins()
	{
		...
		foreach (DayZEditorBuildMenuSubcategoryRegistration subcategory_registration: s_BuiltinSubcategories) {
	...
	protected static void EnsureBuiltinDefinitions()
	{
		...
		s_BuiltinTabs = new array<ref DayZEditorBuildMenuTabRegistration>();
		s_BuiltinSubcategories = new array<ref DayZEditorBuildMenuSubcategoryRegistration>();
	...
	{
		s_BuiltinTabs.Insert(new DayZEditorBuildMenuTabRegistration(id, label, icon, order));
	}
	...
	{
		s_BuiltinSubcategories.Insert(new DayZEditorBuildMenuSubcategoryRegistration(tab_id, id, label, icon, order));
	...
	{
		EditorBuildMenuTabDefinition existing_tab = m_TabById[id];
		if (existing_tab) {
		...
		EditorBuildMenuTabDefinition tab_definition = new EditorBuildMenuTabDefinition(id, label, order, icon);
	...
	void AddSubcategory(string tab_id, string id, string label, string icon = string.Empty, int order = -1)
	{
		...
		EditorBuildMenuSubcategoryDefinition existing_subcategory = m_SubcategoryByKey[key];
		...
		if (!m_TabById[tab_id]) {
			EditorLog.Error("EditorBuildMenuTaxonomy::AddSubcategory missing tab %1 for subcategory %2", tab_id, id);
			return;
		}
		...
		EditorBuildMenuSubcategoryDefinition subcategory_definition = new EditorBuildMenuSubcategoryDefinition(tab_id, id, label, order, icon);
	...
	void AddSection(string tab_id, string subcategory_id, string id, string label = string.Empty, int order = -1)
	{
		if (!HasSubcategory(tab_id, subcategory_id)) {
			EditorLog.Error("EditorBuildMenuTaxonomy::AddSection invalid target %1/%2 for section %3", tab_id, subcategory_id, id);
			return;
		}
		...
		EditorBuildMenuSectionDefinition existing_section = m_SectionByKey[key];
		if (existing_section) {
			if (label != string.Empty) {
		...
		if (label == string.Empty) {
			label = EditorBuildMenuInference.FormatLabel(id);
		}
		...
		m_NextSectionOrderByBucket[bucket_key] = Math.Max(m_NextSectionOrderByBucket[bucket_key], order + 1);
		m_SectionByKey[key] = new EditorBuildMenuSectionDefinition(tab_id, subcategory_id, id, label, order);
	}
	...
	{
		EditorBuildMenuSectionDefinition definition = m_SectionByKey[GetSectionKey(tab_id, subcategory_id, section_id)];
		if (definition) {
			return definition.Label;
		}

		return EditorBuildMenuInference.FormatLabel(section_id);
	}
	...
	array<ref EditorBuildMenuTabDefinition> GetTabs()
	{
		return m_Tabs;
	}

	array<ref EditorBuildMenuSubcategoryDefinition> GetSubcategories()
	{
	...
	{
		ref array<ref EditorBuildMenuTabDefinition> sorted = {};
		foreach (EditorBuildMenuTabDefinition definition: m_Tabs) {
	...
	{
		ref array<ref EditorBuildMenuSubcategoryDefinition> sorted = {};
		foreach (EditorBuildMenuSubcategoryDefinition definition: m_Subcategories) {
	...
	protected void InsertTabSorted(array<ref EditorBuildMenuTabDefinition> definitions, EditorBuildMenuTabDefinition definition)
	{
	...
	protected void InsertSubcategorySorted(array<ref EditorBuildMenuSubcategoryDefinition> definitions, EditorBuildMenuSubcategoryDefinition definition)
	{
		int insert_index = definitions.Count();
		for (int i = 0; i < definitions.Count(); i++) {
	...
	protected bool SortTabBefore(EditorBuildMenuTabDefinition lhs, EditorBuildMenuTabDefinition rhs)
	{
		return CompareOrderedLabels(lhs.Order, lhs.Label, rhs.Order, rhs.Label);
	}

	protected bool SortSubcategoryBefore(EditorBuildMenuSubcategoryDefinition lhs, EditorBuildMenuSubcategoryDefinition rhs)
	{
		if (lhs.TabId != rhs.TabId) {
			EditorBuildMenuTabDefinition lhs_tab = m_TabById[lhs.TabId];
			EditorBuildMenuTabDefinition rhs_tab = m_TabById[rhs.TabId];
